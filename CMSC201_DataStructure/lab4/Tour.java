...
* 10/6/2018
* CMSC 201
* lab4import edu.princeton.cs.algs4.*;
...
public class Tour {
  ...
  public double distance(){
    double distance = 0; Node n = start;
    while(n!=null && n.next!=null){
  ...
  public void insertSmallest(Point p){// insert p using smallest increase heuristic
    ...
    if (size ==0) {start = n; end = n;}
      ...
      double min_increase = n.p.distanceTo(start.p);
      double increase;
      targetNode = start;
      while(n1!=end){
      ...
      if(n.p.distanceTo(end.p) < min_increase){end.next = n; end = n;} // add to end
      else if (targetNode == start){n.next = start; start = n;}// add to start
