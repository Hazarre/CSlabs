...
public class SATbrute {
    ...
    public static void main(String[] args) {
        if (args.length != 1)
            System.out.println("Please, give the file name as a single parameter");
        String fileName = args[0];
        CNFFormula formula = readFormula(fileName);
        Long result = formula.satisfiedBy();
        if (result == null)
            System.out.println(args[0] + ":  unsatisfiable");
        else
            System.out.println(args[0] + ":  " + longToVariablesValues(result, formula.nVariables));
    }
