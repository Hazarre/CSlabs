...
public class SudokuSolver {
	...
	new ArrayList<ArrayList<Integer>>(); // SAT formula
    ...
    */
	...
	try {
	    ...
	    // skip comments
	    while(s.hasNext(commentPattern)) s.nextLine();
    ...
    */
    ...
    /* Take digits x, y and z and create xyz
       PRE: x,y,and z are in {1,2,...,9} */
    ...
    */
	...
	// Constraint 1.B: Each row has each number at most once
	// TODO
	int col2;
    for(row=1;row<=gridLen;row++){
        for(m=1;m<=gridLen;m++){

        for (col=1;col<=gridLen-1;col++) for (col2 = col+1; col2 <= gridLen; col2++){
                clz = new ArrayList<Integer>();
                clz.add(-1*xyzInt(row,col,m));
                clz.add(-1*xyzInt(row,col2,m));
                formula.add(clz);
        }
    ...
    /**
       Constraint 1: Columns must have all digits and no digit more than once.
    */
	...
	// Constraint 2.B : Each column has each number at most once
	// TODO
    int row2;
    for(col=1;col<=gridLen;col++){
        for(m=1;m<=gridLen;m++){

        for (row=1;row<=gridLen-1;row++) for (row2 = row+1; row2<=gridLen; row2++){
                clz = new ArrayList<Integer>();
                clz.add(-1*xyzInt(row,col,m));
                clz.add(-1*xyzInt(row2,col,m));
                formula.add(clz);
            }
    ...
    */
	...
	// Constraint 4.A: Each cell contains at least one number 1 to 9
	ArrayList<Integer> clz = new ArrayList<Integer>();
	int row,col,m=1;
	// TODO
    for(col=1;col<=gridLen;col++){
        for (row=1;row<=gridLen;row++){
            clz = new ArrayList<Integer>();
            for(m=1;m<=gridLen;m++) clz.add(xyzInt(row,col,m));
            formula.add(clz);
        }
    }

	// Constraint 4.B: Each cell contains no more than one number from 1 to 9
	// TODO
    int m2;
    for(col=1;col<=gridLen;col++){
        for (row=1;row<=gridLen;row++){
            for(m=1;m<=gridLen-1;m++) for(m2=m+1;m2<=gridLen;m2++){
                clz = new ArrayList<Integer>();
                clz.add(-1*xyzInt(row,col,m));
                clz.add(-1*xyzInt(row,col,m2));
                formula.add(clz);
            }
            }
    }
    ...
    /**
       Any preset values that appear in a Sudoku puzzle lead to
       additional clauses in its translation.
    */
    public void preset(){
    ...
    /**
       Certifier for the sudoku solution.
       Caution: side effects this.puzzle!
    */
	...
	for (int pos=0;pos<solution.length;pos++) {
		...
		if (puzzle[x-1][y-1] == 0) puzzle[x-1][y-1] = z;
	    }
	}
    ...
    */
	...
	throws ParseFormatException, ContradictionException, TimeoutException {
	...
	System.out.println("\nSolution is " + (valid?"valid":"invalid"));
